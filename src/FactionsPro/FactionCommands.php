    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
	    ...
	    $prefix = $this->plugin->prefs->get("pluginprefix"); //Prefix configurations.
            if (strtolower($command->getName()) === "f") {
                if (empty($args)) {
                    $sender->sendMessage($this->plugin->formatMessage("$prefix."$this->plugin->messages->get("helpmessage")));
                    return true;
                }
                    ///////////////////////////////// WAR /////////////////////////////////
			    ...
			    var_dump($this->plugin->db->query("SELECT * FROM balance;")->fetchArray(SQLITE3_ASSOC));
                    ...
                    if(strtolower($args[0]) == "invite" or strtolower($args[0]) == "inv"){
                        ...
                        if ($this->plugin->isFactionFull($this->plugin->getPlayerFaction($playerName))) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix" $this->plugin->messages->get("invite_facfull")));
                            return true;
                        }
		        ...
		        $this->plugin->db->query("DELETE FROM balance WHERE faction='$args[1]';");
                    ...
                    if (strtolower($args[0]) == "addstrto" or strtolower($args[0]) == "addpower") {
                        if (!isset($args[1]) or ! isset($args[2])) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §bPlease use: §3/f $args[0] <faction> <STR>\n§aDescription: §dAdds STR to a faction."));
                            return true;
                        }
                        if (!$this->plugin->factionExists($args[1])) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §cThe faction named §4$args[1] §cdoesn't exist."));
		    ...
		    if (strtolower($args[0]) == "rmbalto" or strtolower($args[0]) == "rmmoney") {
		       ...
		       if (!isset($args[1]) or ! isset($args[2])) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §bPlease use: §3/f $args[0] <faction> <power>\n§aDescription: §dRemoves Power from a faction."));
                            return true;
                        }
                        if (!$this->plugin->factionExists($args[1])) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §cThe faction named §4$args[1] §cdoesn't exist."));
...
+                           $sender->getPlayer()->teleport(new Position($array['x'], $array['y'], $array['z'], $level));
			...
			if (!$this->plugin->prefs->get("AllowChat")) {
			    $sender->sendMessage($this->plugin->formatMessage("§c/f $args[0] is disabled"));
			    return true;
			}
			if (!($this->plugin->isInFaction($playerName))) {
		...
		////////////////////////////// BALANCE, by primus ;) ///////////////////////////////////////
			...
			$balance = $this->plugin->getBalance($faction);
			$factioninfo = $this->plugin->prefs->get("FactionInfo");
			...
			$maxPlayers = $this->plugin->prefs->get("MaxPlayersPerFaction");
			$balance = $this->plugin->getBalance($faction);
			$myfacmessage = $this->plugin->prefs->get("MyFactionMessage");
	    ...
	    $prefix = $this->plugin->prefs->get("pluginprefix");
            $this->plugin->getServer()->getLogger()->info($this->plugin->formatMessage("$prefix" $this->plugin->messages->get("consolemessage")));
        }
        return true;
    }
