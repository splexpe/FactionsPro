    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
        if ($sender instanceof Player) {
			...
			$this->plugin->updateTag($sender->getName());
                    ...
                    if ($args[0] == "promote") {
                        if (!isset($args[1])) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §aPlease use: §b/f promote <player>"));
                            return true;
                        }
                        if (!$this->plugin->isInFaction($sender->getName())) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §cYou must be in a faction to use this"));
                    ...
                    if ($args[0] == "demote") {
                        if (!isset($args[1])) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §aPlease use: §b/f demote <player>"));
                            return true;
                        }
                        if ($this->plugin->isInFaction($sender->getName()) == false) {
                            $sender->sendMessage($this->plugin->formatMessage("$prefix §cYou must be in a faction to use this"));
                    ...
                    if ($args[0] == "kick") {
                        ...
                        $stmt->execute();
                        $sender->sendMessage($this->plugin->formatMessage("$prefix §dYou successfully kicked §5$args[1]", true));
					...
					/////////////////////////////// CLAIM ///////////////////////////////
                    if(strtolower($args[0]) == "claim" or strtolower($args[0]) == "addclaim"){
		    ...
		    /////////////////////////////// TOP, also by @PrimusLV //////////////////////////
			    ...
			    $this->plugin->addToBalance($faction, $this->plugin->prefs->get("MoneyGainedPerPlayerInFaction"));
		            $inviter = $this->plugin->getServer()->getPlayerExact($array["invitedby"]);
                            if ($inviter !== null){
			    $inviter->sendMessage($this->plugin->formatMessage("$prefix §2$playerName §ajoined the faction", true));
			    $this->plugin->updateTag($sender->getName());
                        } else {
...
+                           $sender->getPlayer()->teleport(new Position($array['x'], $array['y'], $array['z'], $level));
			...
			for ($i = 0; $resultArr = $result->fetchArray(SQLITE3_ASSOC); $i = $i + 1) {
				...
				$p->sendMessage(TextFormat::ITALIC . TextFormat::DARK_AQUA . $message . TextFormat::RESET);
                    ...
                    if(strtolower($args[0]) == "ally" or strtolower($args[0]) == "addally"){
                        if (!isset($args[1])) {
                    ...
                    if(strtolower($args[0]) == "unally" or strtolower($args[0]) == "removeally"){
                        if (!isset($args[1])) {
			...
			$this->plugin->takeFromBalance($fac, $this->plugin->prefs->get("MoneyGainedPerAlly"));
                    ...
                    if(strtolower($args[0]) == "forceunclaim" or strtolower($args[0]) == "funclaim"){
                        if (!isset($args[1])) {
